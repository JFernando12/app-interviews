...
export class StaticQuestionLoader {
  ...
  /**
   * Load questions for AI Engineer topics
   * @param topic - The topic identifier (e.g., 'general')
   * @returns Promise<Question[]> - Array of questions
   */
  static async loadAIEngineerQuestions(topic: string): Promise<Question[]> {
    const cacheKey = `ai-engineer-${topic}`;
    if (this.cache.has(cacheKey)) {
      return this.cache.get(cacheKey)!;
    }
    ...
    try {
      const questionModule = await import(
        `@/lib/data/questions/ai-engineer/${topic}.json`
      );
      const questions: Question[] = questionModule.default;
      ...
      console.error(
        `Failed to load AI Engineer questions for topic ${topic}:`,
        error,
      );
      return [];
    }
  ...
  static getAvailableLanguages(): string[] {
    ...
    return ['general', 'python', 'typescript', 'react', 'nodejs'];
  }

  /**
   * Get all available AI Engineer topics
   * @returns string[] - Array of available topic identifiers
   */
  static getAvailableAIEngineerTopics(): string[] {
    return ['general'];
  }
  ...
  static async preloadQuestions(languages: string[]): Promise<void> {
    const loadPromises = languages.map((lang) =>
      this.loadTechnicalQuestions(lang),
    );
